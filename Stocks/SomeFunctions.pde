  ...
  if(rapid) {
    ...
    for(int i = 0; i < vals.size(); i++) {
      ...
      if(names[i].equals(sym)) {
        symCost = Float.parseFloat(vals.get(sym));
        temp = false;
        break;
      }
    }
    ...
    if(response != null) {
      ...
      symCost = Float.parseFloat((response[6].split(":")[1]));
      stockDisplayed = true;
      stockName = sym;
    } else{
...
void loadStocks() {
  ...
  if(temp.length - 2 != 0) {
    ...
    money = Float.parseFloat(temp[temp.length - 2]);
  }
...
float roundVal(float x) {
  return Float.parseFloat(new DecimalFormat("########.##").format(x));
}